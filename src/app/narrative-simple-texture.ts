...
//            }
            ...
            if(_webvr_skycube){
              ...
              if(_sg3D){
                ...
                // simple test with static loaded image texture
                vr_cube.material.map = texture;
...
//            }
            ...
            if(frame%1000===0){console.log(`frame=${frame} webvr:t post:t-or-f sg3D:t-or-f => vr_scene`)};
            renderer.render(vr_scene, lens);

          }else{ // _webvr:f => hud._post:t 
            // _webvr:f => render scene&post 3D-OUT 
            if(frame%1000===0){console.log(`frame=${frame}: webvr:f post:t sg3D:t-or-f => scene-3D`)};
            renderer.render(scene, lens);
          }
...
class Narrative {
  ...
  initialize(){
    ...
    // test texture
    texture = texture_loader.load('./assets/images/channels/mg_cut01.png',
      // onLoad
      (t) => {
        texture = t;
